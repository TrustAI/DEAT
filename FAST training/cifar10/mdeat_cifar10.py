...
def get_args():
    ...
    parser.add_argument('--lr-schedule', default='cyclic', type=str, choices=['cyclic', 'flat', 'multistep'])
    ...
    parser.add_argument('--alpha', default=12., type=float, help='Step size')
    parser.add_argument('--fname', default='output', type=str)
    parser.add_argument('--out-dir', default='mdeat_out', type=str, help='Output directory')
...
def main():
    ...
    logger.info('Epoch \t Seconds \t LR \t \t Train Loss \t Train Acc  \tMG')
    for epoch in range(args.epochs):
        ...
        logger.info('%d \t %.1f \t \t %.4f \t %.4f \t %.4f \t %.4f',
            epoch, epoch_time - start_epoch_time, lr, train_loss/train_n, train_acc/train_n, grad_mag)
    ...
    logger.info('{:.4f} \t\t {:.4f} \t  {:.4f} \t  {:.4f} \t  {:.4f} \t  {:.4f} \t  {:.4f} \t  {:.4f} \t  {:.4f} \t  {:.4f}'.format(test_loss, test_acc, fgsm_loss, fgsm_acc, pgd20_loss, pgd20_acc, pgd100_loss, pgd100_acc, cw_loss, cw_acc))
