...
def train(args, model, device, train_loader, num_steps, optimizer, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        loss, gard_mg = mtrades_loss(model=model,
                           x_natural=data,
                           y=target,
                           optimizer=optimizer,
                           step_size=step_size,
    ...
    return train_loss, train_n, gard_mg
...
def main():
    ...
    logger.info('Epoch \t Seconds \t LR \t \t Train Loss \t Val Acc\t MG \t PGD Acc')
    for epoch in range(args.epochs):
        ...
        train_loss, train_n, grad_mag = train(args, model, device, train_loader, num_steps, optimizer, epoch)
        ...
        if epoch == 0:
            ...
            increase_threshold = args.gamma * grad_mag
                ...
                or (args.step_size * num_steps < args.epsilon)):
            increase_threshold = args.gamma * grad_mag
            num_steps += 1
