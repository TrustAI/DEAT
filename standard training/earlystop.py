...
def earlystop(model, data, target, step_size, epsilon, perturb_steps,tau,randominit_type,loss_fn,mu,std,rand_init=True,omega=0):
    '''
    ...
    if rand_init:
        if randominit_type == "normal_distribution_randominit":
            iter_delta = 0.001*torch.randn_like(data).cuda()
            iter_delta = clamp(iter_delta, lower_limit - data, upper_limit - data)
            iter_adv = data.detach() + iter_delta
        if randominit_type == "uniform_randominit":
            iter_delta = torch.zeros_like(data).cuda()
    ...
    output_iter_clean_data = model(normalize(data,mu,std))

    while K>0:
        iter_adv.requires_grad_()
        output = model(normalize(iter_adv,mu,std))
        ...
        if len(iter_index) != 0:
            ...
            delta = iter_adv - data[iter_index]
            delta.data = clamp(delta, lower_limit - data[iter_index], upper_limit - data[iter_index])
            iter_adv = data[iter_index].detach() + delta.detach()
