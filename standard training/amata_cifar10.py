...
def get_args():
    ...
    parser.add_argument('--tau', default=20, type=float)
    parser.add_argument('--min-iteration', default=2, type=int)
    parser.add_argument('--max-iteration', default=10, type=int)
    ...
    parser.add_argument('--out-dir', default='amata_out', type=str, help='Output directory')
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        train_acc = 0
        train_n = 0
        cur_iteration = args.min_iteration + int((args.max_iteration - args.min_iteration) * (epoch / args.epochs))
        alpha = tau/cur_iteration
        ...
        logger.info('%d \t %.1f \t \t %.4f \t %.4f \t %.4f \t %.4f\t %.4f',
            epoch, epoch_time, lr, train_loss/train_n, train_acc/train_n, val_acc, val_adv_acc)
