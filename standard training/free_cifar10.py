...
def get_args():
    ...
    parser.add_argument('--out-dir', default='free_out', type=str, help='Output directory')
...
def main():
    ...
    highest_acc = 0
    train_time = 0
    logger.info('Epoch \t Seconds \t LR \t \t Train Loss \t Train Acc \t Val Acc \t CW Acc')
    for epoch in range(args.epochs):
        ...
        if val_adv_acc > highest_acc and args.save_model:
            ...
            highest_idx = epoch
            torch.save(model.state_dict(), os.path.join(args.out_dir, f'model_{args.model}{args.max_iteration}.pth'))
